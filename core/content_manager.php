...
class content_manager {
	...
	public function GetContentTitle() {
		if ($this->provider_section_name) {
			...
			return str_replace("MAIN ", "", $this->section).($this->section == SECTION_THECITIES ? (" (".city_helper::GetCurrentCityName().")") : "");
		}
		else {
	...
	public function GetSectionContentItems() {
		...
		if ($this->content_type) {
			if ($this->q) {
				...
				$content = instance_factory::GetContentInstance($this->content_type);
                $content_list = (isset($content)) ? $content->GetItemsBySection($this->section, false, $this->limit) : array();
			}
		}
		else {
	...
	public function GetViewMoreUrl() {
		...
		return url_handler::GetAbsoluteUrl(($this->q ? "search.php" : "list.php").$url.($this->q ? ("&q=".$this->q) : ""));
	}
	
	public function GetSerieContentItems($serieid, $content_type = null) {
		if ($content_type) {
			$content = instance_factory::GetContentInstance($content_type);
			$content_list = $content->GetItemsBySerie($serieid);
		}
		else {
			$content_film = new content_film_extended();
