<?php
class content_show_extended extends content_show implements icontent {
	public $content_type = CONTENTTYPE_SHOW;
	private $query_helper;
	
	public function content_show_extended() {
		$this->query_helper = new query_helper("content_showid", $this->content_type);
	}
	...
	public function GetItemsBySection($section, $count = false, $limit = null) {
		if ($count) {
			$select = "select count(c.content_showid) as content_count";
		}
		else {
			$select = "select c.content_showid as id, c.title as title, c.buy_price as price, c.overview as overview, c.review_points_users as rating, c.view_count as view_count, c.created as created";
		}
	
		$where = $this->query_helper->GetQueryBySection($section, $limit);
		
		$this->pog_query = $select." from content_show c".$where;
		$connection = Database::Connect();
		$cursor = Database::Reader($this->pog_query, $connection);
		if ($count) {
			$row = Database::Read($cursor);
	...
	public function GetItemsByCategory($categoryid, $count = false) {
		if ($count) {
			...
			$select = "select c.content_showid as id, c.title as title, c.buy_price as price, c.overview as overview, c.review_points_users as rating";
		}
		$this->pog_query = $select." from content_show c where c.active = 1 and c.show_categoryid = ".$categoryid;
		$connection = Database::Connect();
		$cursor = Database::Reader($this->pog_query, $connection);
		if ($count) {
			$row = Database::Read($cursor);
	...
	public function GetItemsByProvider($providerid, $categoryid = null, $count = false, $backend = false, $limit = null) {
		...
		if ($count) {
			...
			$select = "select c.content_showid as id, c.title as title, c.buy_price as price, c.overview as overview, c.review_points_users as rating, c.active as active, c.fileid as fileid, c.rhovit_user_provider_serieid as serieid";
		}
		$where = " where ".$active." c.providerid = ".$providerid;
		if ($categoryid) $where .= " and c.show_categoryid = ".$categoryid;
		$this->pog_query = $select." from content_show c".$where." order by created desc".($limit ? (" limit 0, ".$limit) : "");
		$connection = Database::Connect();
		$cursor = Database::Reader($this->pog_query, $connection);
		if ($count) {
			$row = Database::Read($cursor);
	...
	public function GetPurchasedItems($userid, $count = false) {
		if ($count) {
			...
			$select = "select c.content_showid as id, c.title as title, c.buy_price as price, c.overview as overview, c.review_points_users as rating, up.purchase_type as purchase_type";
		}
		$where = " where c.active = 1";
		$this->pog_query = $select." from content_show c inner join user_purchase up on userid = ".$userid." and c.content_showid = up.contentid and up.content_type = '".$this->content_type."' ".$where." order by up.purchase_date desc";
		
		$query2 = "SELECT c.content_showid AS id, c.title AS title, c.buy_price AS price, c.overview AS overview, c.review_points_users AS rating,  'Single Episode(s) - Buy' AS purchase_type
					FROM content_show c
					INNER JOIN content_show_track mt ON ( c.content_showid = mt.content_showid ) 
					WHERE content_show_trackid
					IN (
						SELECT contentid
						FROM user_purchase 
						WHERE userid = ".$userid."
	...
	public function GetHtmlSlideImages(){
		for($i=0;$i<=3;$i++){
			$f_src = UPLOAD_CONTENT.UPLOAD_CONTENT_SHOW_SUBDIR.$this->content_showId."_slide_".$i.".jpg";
	...
	public function GetTrackList() {
		$content_show_track = new content_show_track();
		return $content_show_track->GetList(array(array("content_showid", "=", $this->content_showId), array("active", "=", 1)), "track_number");
	}
	...
	public function GetItemsBySerie($serieid, $count = false) {
		if ($count) {
			...
			$select = "select c.content_showid as id, c.title as title, c.buy_price as price, c.overview as overview";
		}
	...
	public function GetSerieContentItemsForEdit($providerid, $serieid = null) {
		$this->pog_query = "select c.content_showid as id, c.title as title from content_show c where c.active = 1 and c.providerid = ".$providerid;
		if ($serieid) {
	...
	public function AddToSerie($id, $serieid) {
		$connection = Database::Connect();
		$this->pog_query = "update content_show set rhovit_user_provider_serieid = ".$serieid." where content_showid = ".$id;
		Database::NonQuery($this->pog_query, $connection);
	}
	
	public function UpdateRating() {
		$connection = Database::Connect();
		$this->pog_query = "update content_show set review_points_users = ".$this->review_points_users." where content_showid = ".$this->content_showId;
		Database::NonQuery($this->pog_query, $connection);
	}
	
	public function GetReviewItems($userid) {
		$this->pog_query = "select r.content_reviewid as id, r.contentid as contentid, r.points as points, r.comment as comment, r.review_date as review_date, c.title as title from content_review r inner join content_show c on r.contentid = c.content_showid and r.content_type = '".$this->content_type."' where r.enabled = 1 and r.userid = ".$userid;
		$connection = Database::Connect();
		$cursor = Database::Reader($this->pog_query, $connection);
		$list = array();
		while ($row = Database::Read($cursor)) {
