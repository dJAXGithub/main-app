<?php
class content_game_extended extends content_game implements icontent {
	public $content_type = CONTENTTYPE_GAME;
	private $query_helper;
	
	public function content_game_extended() {
		$this->query_helper = new query_helper("content_gameid", $this->content_type);
	}
	...
	public function GetDisplayNameCategory() {
		$select = "select name FROM game_category WHERE game_categoryid = ".$this->game_categoryid;
	...
	public function GetItemsBySection($section, $count = false, $limit = null) {
		if ($count) {
			$select = "select count(c.content_gameid) as content_count";
		}
		else {
			$select = "select c.content_gameid as id, c.title as title, c.buy_price as price, c.overview as overview, c.review_points_users as rating, c.view_count as view_count, c.created as created";
		}
	
		$where = $this->query_helper->GetQueryBySection($section, $limit);
		
		$this->pog_query = $select." from content_game c".$where;
		$connection = Database::Connect();
		$cursor = Database::Reader($this->pog_query, $connection);
		if ($count) {
			$row = Database::Read($cursor);
	...
	public function GetItemsByCategory($categoryid, $count = false) {
		if ($count) {
			...
			$select = "select c.content_gameid as id, c.title as title, c.buy_price as price, c.overview as overview, c.review_points_users as rating";
		}
		$this->pog_query = $select." from content_game c where c.active = 1 and c.game_categoryid = ".$categoryid;
		$connection = Database::Connect();
		$cursor = Database::Reader($this->pog_query, $connection);
		if ($count) {
			$row = Database::Read($cursor);
	...
	public function GetItemsByProvider($providerid, $categoryid = null, $count = false, $backend = false, $limit = null) {
		...
		if ($count) {
			...
			$select = "select c.content_gameid as id, c.title as title, c.buy_price as price, c.overview as overview, c.review_points_users as rating, c.active as active, c.fileid as fileid, c.rhovit_user_provider_serieid as serieid";
		}
		$where = " where ".$active." c.providerid = ".$providerid;
		if ($categoryid) $where .= " and c.game_categoryid = ".$categoryid;
		$this->pog_query = $select." from content_game c".$where." order by created desc".($limit ? (" limit 0, ".$limit) : "");
		$connection = Database::Connect();
		$cursor = Database::Reader($this->pog_query, $connection);
		if ($count) {
			$row = Database::Read($cursor);
	...
	public function GetPurchasedItems($userid, $count = false) {
		if ($count) {
			...
			$select = "select c.content_gameid as id, c.title as title, c.buy_price as price, c.overview as overview, c.review_points_users as rating, up.purchase_type as purchase_type";
		}
		$where = " where c.active = 1";
		$this->pog_query = $select." from content_game c inner join user_purchase up on userid = ".$userid." and c.content_gameid = up.contentid and up.content_type = '".$this->content_type."' ".$where." order by up.purchase_date desc";
		$connection = Database::Connect();
		$cursor = Database::Reader($this->pog_query, $connection);
		if ($count) {
			$row = Database::Read($cursor);
	...
	public function GetHtmlSlideImages(){
		for($i=0;$i<=3;$i++){
			$f_src = UPLOAD_CONTENT.UPLOAD_CONTENT_GAME_SUBDIR.$this->content_gameId."_slide_".$i.".jpg";
	...
	public function GetItemsBySerie($serieid, $count = false) {
		if ($count) {
			...
			$select = "select c.content_gameid as id, c.title as title, c.buy_price as price, c.overview as overview";
		}
	...
	public function GetSerieContentItemsForEdit($providerid, $serieid = null) {
		$this->pog_query = "select c.content_gameid as id, c.title as title from content_game c where c.active = 1 and c.providerid = ".$providerid;
		if ($serieid) {
	...
	public function AddToSerie($id, $serieid) {
		$connection = Database::Connect();
		$this->pog_query = "update content_game set rhovit_user_provider_serieid = ".$serieid." where content_gameid = ".$id;
		Database::NonQuery($this->pog_query, $connection);
	}
	
	public function UpdateRating() {
		$connection = Database::Connect();
		$this->pog_query = "update content_game set review_points_users = ".$this->review_points_users." where content_gameid = ".$this->content_gameId;
		Database::NonQuery($this->pog_query, $connection);
	}
	
	public function GetReviewItems($userid) {
		$this->pog_query = "select r.content_reviewid as id, r.contentid as contentid, r.points as points, r.comment as comment, r.review_date as review_date, c.title as title from content_review r inner join content_game c on r.contentid = c.content_gameid and r.content_type = '".$this->content_type."' where r.enabled = 1 and r.userid = ".$userid;
		$connection = Database::Connect();
		$cursor = Database::Reader($this->pog_query, $connection);
		$list = array();
		while ($row = Database::Read($cursor)) {
